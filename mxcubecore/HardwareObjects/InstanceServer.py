...
# from QtImport import qt_variant, QApplication
...
<procedure class="InstanceServer">
  ...
  <check_method>ssh</check_method>
  <ssh_host>collect1</ssh_host>
</procedure>
"""
...
class InstanceServer(Procedure):
    ...
    def init(self):
        ...
        self.checkMethod = self.getProperty('check_method', 'display')
        ssh_host = self.getProperty('ssh_host', 'localhost')
        self.ssh_hosts = ssh_host.split(',')
        remote_auth_users = self.getProperty('remote_auth_users', '')
        self.remote_auth_users = remote_auth_users.split(",")
    ...
    def startServer(self):
        if self.asyncServer is not None:
            ...
            try:
                ...
                log.warning('--> InstanceServer: cannot create server, so trying to connect to it')
                self.connectToServer()
            else:
                self.asyncServer=async_server
                server_hostname=self.serverHost.split('.')[0]
                log.debug('--> InstanceServer: listening to connections on %s:%d' % (server_hostname,self.serverPort))
            ...
            log.error('InstanceServer: not property configured to start the server')
    ...
    def reconnect(self,quiet=False, client_id=None):
        try:
            self.instanceClient = InstanceClient(self.serverHost,self.serverPort)
        except:
            self.instanceClient = None
            if not quiet:
               log.error('--> InstanceServer: cannot connect to server')
    ...
    def isNoMachine(self):
        # display something like :1002
        display=os.environ["DISPLAY"]
        user=os.environ["USER"]
        if user in self.remote_auth_users:
            return False
        else:
            return re.match("\:10\d+", display) is not None

    def isLocal(self):    
        if self.checkMethod == 'ssh':
            origin=os.environ.get("SSH_CLIENT",None)
            #logging.getLogger("isLocal: %s(%s) vs %s" % (socket.getfqdn(origin), origin, str(self.ssh_hosts)))

            if origin: 
                return socket.getfqdn(origin) in self.ssh_hosts
            else:
                # We suppose that this means that we are running
                # on local computer.
                # We suppose also that the serverHost is our computer
                #  supposing too much?
                return True
        else:
            try:
            ...
            if not display_host:
                log.debug("--> InstanceConnection -  isLocal() = True")
                return True

            return socket.getfqdn(display_host)==self.serverHost
    ...
    def sendChatMessage(self,priority,message, client_id=None):
        ...
        if self.isServer():
            broadcast_to_clients(data)
        elif self.isClient() or (client_id is not None):
            send_data_to_server(self.instanceClient, data)
            my_id=self.clientId2
        else:
...
def handleRemoteClient(client_socket, addr):
  ...
  log.debug("handling remote client %s - %s" % (str(client_socket), str(addr)))
  ...
  while True:
    try:
        ...
        log.debug("Could not get data from InstanceClient. connection lost? %s" % str(e))

    if data == "":
      log.debug("  - empty data received, closing")
      ...
      try:
        ...
        log.debug("  - not in server_clients? %s" % str(e))
