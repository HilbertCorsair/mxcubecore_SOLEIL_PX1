...
class TangoDCMotor(HardwareObject):
    ...
    def set_phase(self, phase):
        if not phase in ["VISU_SAMPLE", "DEFAULT"]:
            print("Allowed phase for backlight are VISU_SAMPLE or DEFAULT")
            raise Exception
        self.environment.go_to_phase(phase)

    def phase_switch(self):
        if self.environment.is_phase_visu_sample():
            self.set_phase("DEFAULT")
        else:
            self.set_phase("VISU_SAMPLE")
