...
class PX1MiniDiff(GenericDiffractometer):
    ...
    grid_direction = {"fast": (1, 0), "slow": (0, 1), "omega_ref": 0}
    ...
    def init(self):
        ...
        self.centring_methods = {
            ...
            GenericDiffractometer.CENTRING_METHOD_AUTO: self.px1_automatic_centring,
    ...
    def px1_center(
        ...
        try:
            ...
            logging.getLogger("HWR").debug(
                "sample_centring:   beam_xc = %s, beam_yc = %s " % (beam_xc, beam_yc)
            )
            logging.getLogger("HWR").debug(
    ...
    def move_pin_length(self):
        try:
            ...
            if abs(goto) > 4:
                logging.getLogger("HWR").debug(
                    " pin length position %s is maybe too big?" % goto
                )
                return
    ...
    def _update_zoom_calibration(self):
        ...
        if props is None:
            logging.getLogger("HWR").debug(
                "PX1MiniDiff. no valid zoom position. calibration is invalid"
            )
            return
        ...
        if "arrowStep" in props.keys():
            self.arrow_step = (
                float(props["arrowStep"]) / 1000
            )  # in zoom.xml value is in microns
        else:
            self.arrow_step = self.default_arrow_step
