...
class PX1Beam(AbstractBeam):
    ...
    def evaluate_beam_info(self):
        ...
        _size = self._definer.beam_config["40 x 20 um"] # as a tuple
    ...
    def _get_aperture_value(self):
        ...
        return eval(self.get_property("aperture"))[0] #[_size, _size], _label
    ...
    def get_value(self):
        ...
        _label = "UNKNOWN"
        if self.aperture:
            _size = self._get_aperture_value()
            _name = "40"
    ...
    def get_beam_position_on_screen(self):
        if self._beam_position_on_screen == [0, 0]:
            self._beam_info.init()
            try:
                _beam_position_on_screen = (
                self._beam_info.get_beam_position()#HWR.beamline.diffractometer.get_beam_position()
                )
            except AttributeError:
                _beam_position_on_screen = (
