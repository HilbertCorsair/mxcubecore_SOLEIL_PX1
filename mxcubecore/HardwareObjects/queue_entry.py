...
class SampleQueueEntry(BaseQueueEntry):
    ...
    def execute(self):
        ...
        if len(self.get_data_model().get_children()) != 0 and sc_used:
            ...
            elif self.beamline_setup.in_chip_mode():
                mount_device = self.beamline_setup.chip_manager_hwobj
            else:
                mount_device = self.sample_changer_hwobj

            if mount_device is not None:
                log.info("Loading sample " + self._data_model.loc_str)
                log.info("checking if sample %s is mounted" % str(self._data_model.location))
...
class DataCollectionQueueEntry(BaseQueueEntry):
    ...
    def pre_execute(self):
        ...
        logging.getLogger("HWR").debug("queue_entry.py - PRE_EXECUTE collect.  connecting to hwobjects 6")
        try:
...
#            self.parallel_processing_hwobj.done_event.clear()
        ...
        sample = self.get_data_model().get_parent().get_parent()
        logging.getLogger("user_level_log").info('Collection finished for sample=%s' % str(sample.location))
        try:
            self.beamline_setup.sample_changer_hwobj.set_sample_collected(sample.location)
        except:
            import traceback
            log.debug("cannot flag sample as collected")
            log.debug(traceback.format_exc())
        logging.getLogger("HWR").debug('queue_entry: data collection finished')
...
class CharacterisationGroupQueueEntry(BaseQueueEntry):
    ...
    def pre_execute(self):
        BaseQueueEntry.pre_execute(self)
        logging.getLogger("HWR").debug('queue_entry: execution of the CharacterisationGroupQueueEntry')
        ...
        logging.getLogger("HWR").debug('queue_entry: using edna = %s', self.use_edna)
        ...
        char.run_characterisation = self.use_edna
    ...
    if hasattr(sample_mount_device, '__TYPE__'):
        if sample_mount_device.__TYPE__ in ['Marvin','CATS']:
            ...
            if wash:
                ...
                sample_mount_device.wash(wait=True)
                sample_mount_device.waitReady(timeout=10)
                if sample_mount_device.isReady():
                    diffractometer_device.move_motors(saved_diffr_positions)
                    gevent.sleep(0.2)
