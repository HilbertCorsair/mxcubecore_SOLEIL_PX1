...
class ISPyBClient2(HardwareObject):
    ...
    def login(self, loginID, psd, ldap_name=None, ldap_connection=None):
        ...
        if not prop_ok:
            # allow for user to collect even if no ISPyB connection. pass proposal information back to Brick
            ...
            if self.loginTranslate is True:
                code = self.translate(proposal_code,'ispyb')
            else:
                code = proposal_code
...
#             group.detectorMode = modes[det_mode]
    ...
    def from_data_collect_parameters(ws_client, mx_collect_dict):
        ...
        try:
            data_collection.imageSuffix = mx_collect_dict['fileinfo']['imageSuffix']
        except KeyError:
            pass
        try:
