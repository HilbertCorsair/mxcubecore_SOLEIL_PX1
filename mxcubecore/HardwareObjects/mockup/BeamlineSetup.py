...
class BeamlineSetup(HardwareObject):
    ...
    def set_chip_mode(self, state):
        ...
        :type state: bool
        """
        self._chip_mode = state
        try:
            self.diffractometer_hwobj.set_chip_mode(state)
        except:
            pass
    ...
    def shutterless_display(self):
        return False

    def use_osc_start_cbox(self):
        return False
    ...
    def _get_resolution_limits(self, energy=None, lightin=False):
        try:
            limits = self.resolution_hwobj.get_resolution_limits(energy, lightin)
            return limits
        except:
            import traceback
    ...
    def close_and_quit(self):
        return "closing from beamline setup"
