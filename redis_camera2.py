...
class camera(object):
    ...
    def get_exposure(self):
        print "Use redis: %s" % self.use_redis # True
        print "Master: %s" % self.master # True
        print "Exposure: %.9f" % (self.camera.ExposureTimeAbs/1.e6)
        if not self.use_redis:
    ...
    def set_exposure_time(self, exposure_time):
        print "Set Exposure: %.9f" % (exposure_time)
    ...
    def run_camera(self):
        ...
        while self.master:
            ...
            if self.frame0._frame.frameID != last_frame_id:
                ...
                # Encode the image as JPEG before base64 encoding
                _, encoded_img = cv2.imencode('.jpg', img)
                image_dict = {
                        "data": base64.b64encode(encoded_img.tostring()).decode('utf-8'),
                        "size": img.shape[:2]}  # Only height and width
                self.redis.publish("mxcubeweb", json.dumps(image_dict))
